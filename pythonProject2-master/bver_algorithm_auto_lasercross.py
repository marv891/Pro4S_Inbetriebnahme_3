...
def mainLaserCrossDetection(laserImage):
    ...
    img_filtered = cv.GaussianBlur(value, (0, 0), 1)
    
    
    dst = cv.Canny(img_filtered, 50, 100, None, 3)
    
    # Copy edges to the images that will display the results in BGR
    cdst = cv.cvtColor(dst, cv.COLOR_GRAY2BGR)
    
    lines = cv.HoughLines(dst, 1, np.pi / 1, 150, 50, 0, 0)
    s = []
